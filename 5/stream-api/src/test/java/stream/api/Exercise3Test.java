...
public class Exercise3Test extends ClassicOnlineStore {
    ...
    public void howManyItemsWanted() {
        ...
        sum = customerList.stream().flatMap(getItemStream).map(Item::getName).count();
    ...
    public void richestCustomer() {
        ...
        Optional<Integer> richestCustomer = customerList.stream().map(Customer::getBudget).sorted(comparator).reduce((a,b) -> b);
    ...
    public void youngestCustomer() {
         ...
         * Don't use {@link Stream#sorted}
         */
        Comparator<Customer> comparator = (a, b) -> a.getAge().compareTo(b.getAge());
